...
Below, I provide some discussion on some of the techniques employed in the network, and discuss how they are applicable for our scenario. Since many of the following were also covered in the lecture, I will not labor the details.

#### Fully Convolutional Networks

In order to discuss fully convolutional networks, it is critical to first mention fully connected layers, where a layer might be composed of a single matrix multiplication. By the nature of matrix multiplication, such architectures require a fixed-size input; this means that the network cannot cope with inputs that are bigger or smaller than its original configuration; in some scenarios, global pooling layers before the fully connected layer can also standardize the input to a fixed size -- although this requires a specific application, such as classification. In many cases, input images are resized to match the training samples.

However, this may not be desirable, as resizing images comes at a cost, such as loss of information (as images are typically downsized, in such cases). Moreover, fully connected layers fail to take advantage of local spatial correlation, where a convolution excels. Because a fully convolutional network avoids many of aforementioned issues, it has become common practice in the field.

#### 1x1 Convolution

One type of convolution that our network makes heavy use of is a 1x1 convolution. This is a fairly common technique in recent works on neural networks that is most effective for reducing the number of features. Here's an illustration from a quick google search:
...
![figs/dsc.jpg](figs/dsc.jpg)

Initially proposed in networks such as [Xception](https://arxiv.org/abs/1610.02357) and [MobileNet](https://arxiv.org/abs/1704.04861), (Depthwise) separable convolutions are two-step composition of convolutions where the initial stage considers each channel independently, whose results are merged through the second stage that performs 1x1 convolutions. The two steps are called `depthwise` and `pointwise` convolutions, respectively, based on the operation that they perform.

### Architecture Summary

Given the relatively simplistic environment and a fairly naive implementation, the network did indeed converge rather quickly, and from the video we can see that it does a fair job apart from occasional false positives at a distance. This trait makes me think that the network may perhaps have a more difficult time training for smaller targets, such as a dog or a cat -- most likely worse at a distance, and probably takes more time to initialize the following process.

With the current dataset, it would obviously not work for targets who are not visually congruent to the one currently specified in the dataset. If it were trained on other targets, however, with the appropriate labels, I believe that the network will indeed be able to follow other objects such as a dog or a cat, albeit probably at a worse quality.
...
One of the comments on the previous submission mentioned employing attention mechanisms as part of possible enhancements on the network to improve target localization and segmentation. While this is an enticing idea, it would be quite an involved process; as I'm still in the process of trying to grasp RNNs and related architectures, this was out of scope for this project.

I have also explored with different methods of upsampling steps during the decoding step, including the provided method (bilinear upsampling), converting depth channels to space, and transposed convolutions (often erroneously referred to as deconvolutions). As the network was already fairly powerful, some of these techniques made the network unnecessarily heavy; others did not particularly improve the performance drastically, and was thus scrapped.
